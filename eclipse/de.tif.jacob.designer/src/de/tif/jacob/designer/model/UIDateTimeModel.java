 ...
 *
 */
public class UIDateTimeModel extends  UIGroupElementModel implements UIInputElementModel, UIICaptionProviderModel, IFontProviderModel
{
	private UICaptionModel captionModel ;

	public UIDateTimeModel()
	{
	  ...
	  TimestampInput         date   = new TimestampInput();
   ...
   * 
   */
  public UIDateTimeModel(JacobModel jacob, UIGroupContainer container, UIGroupModel group, CastorGuiElement guiElement)
  {
  ...
  {
    getCastorTimestampInput().setCaption(caption);
  }
  
  
  public CastorCaption getCastorCaption()
  {
    return getCastorTimestampInput().getCaption();
  }

  @Override
  public CastorDimension getCastorDimension()
  {
    return getCastorTimestampInput().getDimension();
  }
  ...
  public boolean isI18NKeyInUse(String key)
  {
	...
	public void setSize(Dimension size)
	{
		...
		getCastorTimestampInput().getDimension().setHeight(size.height);
		getCastorTimestampInput().getDimension().setWidth(size.width);
		firePropertyChange(PROPERTY_CONSTRAINT_CHANGED, save, size);
	}

	public Point getLocation()
	{
	  return new Point(getCastorTimestampInput().getDimension().getX(), getCastorTimestampInput().getDimension().getY());
	}
	
	public Dimension getSize()
	{
	  return new Dimension(getCastorTimestampInput().getDimension().getWidth(), getCastorTimestampInput().getDimension().getHeight());
	}
  ...
  {
    if(getCastorTimestampInput().getFont()==null)
...
//    firePropertyChange(PROPERTY_ALIGN_CHANGED, save, align);
  ...
  private void checkFontDefault()
  {
    ...
    CastorFont font = getCastorTimestampInput().getFont();
    ...
    if(!font.getWeight().equals(FontDefinition.DEFAULT_FONT_WEIGHT))
      return;
    getCastorTimestampInput().setFont(null);
  ...
  private CastorFont getCastorFont()
  {
    if(getCastorTimestampInput().getFont()==null)
    {
      ...
      getCastorTimestampInput().setFont(font);
    }
    return getCastorTimestampInput().getFont();
  }

	private TimestampInput getCastorTimestampInput()
	{
	  return getCastor().getCastorGuiElementChoice().getLocalInputField().getTimestampInput();
	}
  ...
  {
    return "IDateTimeEventHandler.java";
  }
}
