 ...
 *
 */
public class UIDBListboxModel extends UIAbstractListboxModel implements UIDBLocalInputFieldModel
{
  public UIDBListboxModel()
  {
    super();
  }

  protected UIDBListboxModel(JacobModel jacob, UIGroupContainer container,  UIGroupModel group, CastorGuiElement guiElement)
  {
  ...
  public IPropertyDescriptor[] getPropertyDescriptors()
	...
	public void setField(String fieldName)
	{
	  ...
	  getCastorListBox().removeAllValue();
	  String[] enumValues=getFieldModel().getEnumFieldType().getEnumValues();
	  for(int i=0;i<enumValues.length;i++)
	  {
	    setEnumValueVisible(enumValues[i],true);
	...
	{
	  for(int i=0;i<getCastorListBox().getValueCount();i++)
	  {
	    if(getCastorListBox().getValue(i).equals(enumValue))
	      return true;
	  }
	  return false;
	}
	  ...
	  for(int i=0;i<getCastorListBox().getValueCount();i++)
	  {
	    if(getCastorListBox().getValue(i).equals(enumValue))
	    {
	      if(visibleFlag==false)
	      	getCastorListBox().removeValue(i);
//        return;
	    }
	  }
	  if(visibleFlag==true)
	  	getCastorListBox().addValue(enumValue);
	}
  ...
  {
    return "IListBoxEventHandler.java";
  }

  public String getError()
  {
    if(getFieldModel() == getFieldModel().getTableModel().NULL_FIELD)
      return "Listbox ["+getName()+"] has not a valid reference to a column in the table ["+getFieldModel().getTableModel().getName()+"]";
    return null;
  }
