 ...
 *
 */
public class UIDBEnumComboboxModel extends UIComboboxModel implements UIDBLocalInputFieldModel
{
  public UIDBEnumComboboxModel()
  {
    super();
  }

  protected UIDBEnumComboboxModel(JacobModel jacob, UIGroupContainer container,  UIGroupModel group, CastorGuiElement guiElement)
  {
...
//			descriptors[superDescriptors.length+i+1] = new ComboBoxPropertyGroupingDescriptor(ID_PROPERTY_ENUMVALUE+i, enumValues[i],new String[]{"hidden","visible"}, PROPERTYGROUP_ENUM);
	...
	public void setField(String fieldName)
	{
	  ...
	  getCastorComboBox().removeAllValue();
	  String[] enumValues=getFieldModel().getEnumFieldType().getEnumValues();
	  for(int i=0;i<enumValues.length;i++)
	  {
	    setEnumValueVisible(enumValues[i],true);
	...
	{
	  for(int i=0;i<getCastorComboBox().getValueCount();i++)
	  {
	    if(getCastorComboBox().getValue(i).equals(enumValue))
	      return true;
	  }
	  return false;
	}
	  ...
	  for(int i=0;i<getCastorComboBox().getValueCount();i++)
	  {
	    if(getCastorComboBox().getValue(i).equals(enumValue))
	    {
	      if(visibleFlag==false)
	        getCastorComboBox().removeValue(i);
//        return;
	    }
	  }
	  if(visibleFlag==true)
	    getCastorComboBox().addValue(enumValue);

    // Wichtig: Sortierreihenfolge an das Datenbankfeld anpassen
    //
    List values = Arrays.asList(getCastorComboBox().getValue());
    String dbValues[] = getFieldModel().getEnumFieldType().getEnumValues();
    getCastorComboBox().removeAllValue();
    for (String string : dbValues)
    {
      if(values.contains(string))
        getCastorComboBox().addValue(string);
    }
