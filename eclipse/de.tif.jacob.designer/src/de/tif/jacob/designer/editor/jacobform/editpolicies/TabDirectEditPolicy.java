...
import de.tif.jacob.designer.editor.jacobform.commands.ChangeTabLabelCommand;
 ...
 * @author Phil Zoio
 */
public class TabDirectEditPolicy extends DirectEditPolicy
{
	...
	{
		UITabModel tab = (UITabModel) getHost().getModel();

		Command cmd=null;
		CellEditor cellEditor = request.getCellEditor();
		String oldValue = tab.getLabel();
		String newValue = (String) cellEditor.getValue();
		
	  if(tab.getJacobModel().getTestResourcebundle()==null || !tab.getLabel().startsWith("%"))
	  {
		  cmd = new ChangeTabLabelCommand(tab, oldValue, newValue);
	  }
	  else
	  {
	    I18NResourceModel model = tab.getJacobModel().getTestResourcebundle();
	    String key = tab.getLabel().substring(1);
