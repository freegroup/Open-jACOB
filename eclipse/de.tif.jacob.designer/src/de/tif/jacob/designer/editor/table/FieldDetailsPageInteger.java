...
import de.tif.jacob.designer.model.FieldModelTypeInteger;
 ...
 * Window - Preferences - Java - Code Generation - Code and Comments
 */
public class FieldDetailsPageInteger extends FieldDetailsPageCommon
{
  ...
  protected void createAdditionalContents(FormToolkit toolkit, Composite parent)
  {
    ...
    section.setText("Integer field details");
    section.setDescription("Set the detail properties of the selected integer field.");
      ...
      public void modifyText(ModifyEvent e)
      {
        try
        {
          if (defaultValueText.getText().length() == 0)
            getFieldModel().getIntegerFieldType().deleteDefaultValue();
          else
            getFieldModel().getIntegerFieldType().setDefaultValue(Integer.parseInt(defaultValueText.getText()));
        }
      ...
      public void modifyText(ModifyEvent e)
      {
        try
        {
          if (minValueText.getText().length()==0)
            getFieldModel().getIntegerFieldType().deleteMinValue();
          else
            getFieldModel().getIntegerFieldType().setMinValue(Integer.parseInt(minValueText.getText()));
        }
        catch (NumberFormatException ex)
        {
          // ignore, could temporary happen
      ...
      public void modifyText(ModifyEvent e)
      {
        try
        {
          if (maxValueText.getText().length()==0)
            getFieldModel().getIntegerFieldType().deleteMaxValue();
          else
            getFieldModel().getIntegerFieldType().setMaxValue(Integer.parseInt(maxValueText.getText()));
        }
        catch (NumberFormatException ex)
        {
          // ignore, could temporary happen
  ...
  public void refresh()
  {
    ...
    FieldModelTypeInteger myType = getFieldModel().getIntegerFieldType();
    
    this.autoincrementCheckbox.setSelection(myType.getAutoincrement());
    
    String defaultValue = myType.hasDefaultValue()?Integer.toString(myType.getDefaultValue()):"";
    if (!this.defaultValueText.getText().equals(defaultValue))
      this.defaultValueText.setText(defaultValue);

    String minValue = myType.hasMinValue()?Integer.toString(myType.getMinValue()):"";
    if (!this.minValueText.getText().equals(minValue))
      this.minValueText.setText(minValue);

    String maxValue = myType.hasMaxValue()?Integer.toString(myType.getMaxValue()):"";
