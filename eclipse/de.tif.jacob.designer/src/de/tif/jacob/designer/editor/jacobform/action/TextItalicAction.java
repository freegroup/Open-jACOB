...
import de.tif.jacob.designer.editor.jacobform.commands.ChangeFontStyleCommand;
import de.tif.jacob.designer.editor.jacobform.commands.ChangeFontWeightCommand;
...
public class TextItalicAction implements IEditorActionDelegate 
{
  ...
  public final void setActiveEditor(IAction action, IEditorPart targetEditor)
  {
    ...
    if(((IStructuredSelection)selection).getFirstElement()!=null)
    {
      ...
      while (iter.hasNext())
      {
        ...
        if(anyModel instanceof IFontProviderModel)
        {
          ...
          if(model.getFontStyle().equals(ObjectModel.FONTSTYLE_ITALIC))
            checked=true;
          elements.add(model);
        }
      }
    ...
    String style = model.getFontStyle().equals(ObjectModel.FONTSTYLE_ITALIC)?ObjectModel.FONTSTYLE_NORMAL:ObjectModel.FONTSTYLE_ITALIC;
    cmd = new ChangeFontStyleCommand(model, style);
    command.add(cmd);
    
    while (iterator.hasNext())
    {
      model = (IFontProviderModel)iterator.next();
      cmd = new ChangeFontStyleCommand(model, style);
      command.add(cmd);
    }
