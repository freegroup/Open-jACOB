...
import de.tif.jacob.designer.model.FieldModelTypeDouble;
 ...
 *  
 */
public class FieldDetailsPageDouble extends FieldDetailsPageCommon
{
  ...
  protected void createAdditionalContents(FormToolkit toolkit, Composite parent)
  {
    ...
    section.setText("Double field details");
    section.setDescription("Set the detail properties of the selected double field.");
      ...
      public void modifyText(ModifyEvent e)
      {
        try
        {
          if (defaultValueText.getText().length() == 0)
            getFieldModel().getDoubleFieldType().deleteDefaultValue();
          else
            getFieldModel().getDoubleFieldType().setDefaultValue(Double.parseDouble(defaultValueText.getText()));
        }
    ...
    this.defaultValueText.addVerifyListener(Verify.DOUBLE);
      ...
      public void modifyText(ModifyEvent e)
      {
        try
        {
          if (minValueText.getText().length() == 0)
            getFieldModel().getDoubleFieldType().deleteMinValue();
          else
            getFieldModel().getDoubleFieldType().setMinValue(Double.parseDouble(minValueText.getText()));
        }
        catch (NumberFormatException ex)
        {
          // ignore, could temporary happen
    ...
    this.minValueText.addVerifyListener(Verify.DOUBLE);
      ...
      public void modifyText(ModifyEvent e)
      {
        try
        {
          if (maxValueText.getText().length() == 0)
            getFieldModel().getDoubleFieldType().deleteMaxValue();
          else
            getFieldModel().getDoubleFieldType().setMaxValue(Double.parseDouble(maxValueText.getText()));
        }
        catch (NumberFormatException ex)
        {
          // ignore, could temporary happen
    ...
    this.maxValueText.addVerifyListener(Verify.DOUBLE);
  ...
  public void propertyChange(PropertyChangeEvent evt)
  {
    ...
    {
      if (evt.getNewValue() != FieldModel.DBTYPE_DOUBLE)
  ...
  public void refresh()
  {
    ...
    FieldModelTypeDouble myType = getFieldModel().getDoubleFieldType();

    String defaultValue = myType.hasDefaultValue() ? Double.toString(myType.getDefaultValue()) : "";
    if (!this.defaultValueText.getText().equals(defaultValue))
      this.defaultValueText.setText(defaultValue);

    String minValue = myType.hasMinValue() ? Double.toString(myType.getMinValue()) : "";
    if (!this.minValueText.getText().equals(minValue))
      this.minValueText.setText(minValue);

    String maxValue = myType.hasMaxValue() ? Double.toString(myType.getMaxValue()) : "";
