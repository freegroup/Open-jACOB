...
public class ButtonDirectEditPolicy extends DirectEditPolicy
{
	...
	{
    Command cmd=null;
    UIButtonModel button = (UIButtonModel) getHost().getModel();
    CellEditor cellEditor = request.getCellEditor();
    String oldValue = button.getLabel();
    String newValue = (String) cellEditor.getValue();
    
    if(button.getJacobModel().getTestResourcebundle()==null || !button.getLabel().startsWith("%"))
    {
      ChangeButtonLabelCommand textcmd = new ChangeButtonLabelCommand();
      textcmd.setButtonModel(button);
    ...
    {
      I18NResourceModel model = button.getJacobModel().getTestResourcebundle();
      String key = button.getLabel().substring(1);
