...
public class TextBoldAction implements IEditorActionDelegate 
{
  ...
  public final void setActiveEditor(IAction action, IEditorPart targetEditor)
  {
    ...
    if(((IStructuredSelection)selection).getFirstElement()!=null)
    {
      ...
      while (iter.hasNext())
      {
        ...
        if(anyModel instanceof IFontProviderModel)
        {
          ...
          if(model.getFontWeight().equals(ObjectModel.FONTWEIGHT_BOLD))
            checked=true;
          elements.add(model);
        }
      }
    ...
    String weight = model.getFontWeight().equals(ObjectModel.FONTWEIGHT_BOLD)?ObjectModel.FONTWEIGHT_NORMAL:ObjectModel.FONTWEIGHT_BOLD;
    cmd = new ChangeFontWeightCommand(model, weight);
    command.add(cmd);
