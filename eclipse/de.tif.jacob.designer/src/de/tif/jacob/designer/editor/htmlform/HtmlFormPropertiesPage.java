 ...
 * Comments
 */
public class HtmlFormPropertiesPage extends FormPage
{
  ...
  public HtmlFormPropertiesPage(FormEditor editor)
  {
  ...
  protected void createFormContent(IManagedForm managedForm)
  {
    ...
    HtmlFormEditorInput jacobInput = (HtmlFormEditorInput) getEditorInput();
    form.setText("Html Form: "+jacobInput.getFormModel().getName());
  ...
  public void init(IEditorSite site, IEditorInput input)
  {
    ...
    HtmlFormEditorInput jacobInput = (HtmlFormEditorInput) input;
  }
  ...
  private void createTableAliasSection(final ScrolledForm form, FormToolkit toolkit)
  {
    ...
    aliasCombo.addSelectionListener(new SelectionListener(){
      ...
      {
        getHtmlFormModel().setTableAlias(((Combo)e.widget).getText());
      }
    ...
    browserCombo.addSelectionListener(new SelectionListener(){
      public void widgetSelected(SelectionEvent e)
      {
        getHtmlFormModel().setBrowser(((Combo)e.widget).getText());
      }
      public void widgetDefaultSelected(SelectionEvent e)
      {
        throw new UnsupportedOperationException();
    ...
    groupEventHandlerHyperlink.addHyperlinkListener(new HyperlinkAdapter()
    {
      public void linkActivated(HyperlinkEvent e)
      {
        new ShowObjectModelHookAction()
        {
          public ObjectModel getObjectModel()
          {
            return getHtmlFormModel().getRenderGroup();
          }
        }.run(null);
        update();
      }
  ...
  {
    UIHtmlFormModel model = getJacobInput().getFormModel();
    ...
    if (model != null)
    {
      ...
      if(assistant==null)
        assistant= LabelContentAssistant.createContentAssistant(labelText, this.getHtmlFormModel().getJacobModel());

      
      aliasCombo.setItems(getHtmlFormModel().getJacobModel().getTableAliasNames().toArray(new String[0]));
      if (!StringUtil.saveEquals(getHtmlFormModel().getTableAlias(),this.aliasCombo.getText()))
        this.aliasCombo.setText(getHtmlFormModel().getTableAlias());

      browserCombo.setItems(getHtmlFormModel().getJacobModel().getBrowserNames(getHtmlFormModel().getTableAliasModel()).toArray(new String[0]));
      if (!StringUtil.saveEquals(getHtmlFormModel().getBrowser(),this.browserCombo.getText()))
        this.browserCombo.setText(getHtmlFormModel().getBrowser());
      
      formEventHandlerHyperlink.setText(getHtmlFormModel().getHookClassName()==null?"<unset>":getHtmlFormModel().getHookClassName());
      formEventHandlerHyperlink.layout(true);

      groupEventHandlerHyperlink.setText(getHtmlFormModel().getRenderGroup().getHookClassName()==null?"<unset>":getHtmlFormModel().getRenderGroup().getHookClassName());
  ...
  protected UIHtmlFormModel getHtmlFormModel()
  {
    return this.getJacobInput().getFormModel();
  }
  
  private HtmlFormEditorInput getJacobInput()
  {
    return (HtmlFormEditorInput) getEditorInput();
  }
}