...
DecisionEnum.prototype.setWorkflow=function(/*:Workflow*/ workflow)
{
  ...
  {
      this.outputPorts.get(i).setWorkflow(this.workflow);
      this.addPort(this.outputPorts.get(i),this.getWidth(),25+25*i);
  }
}
...
DecisionEnum.prototype.setEnumValues=function(/*:String[]*/ values)
{
  ...
  for(var i = 0; i< this.outputPorts.getSize();i++)
  {
    ...
    this.html.removeChild(this.outputPorts.get(i).decoLabel.getHTMLElement());
    // Port entfernen
    this.removePort(this.outputPorts.get(i));
  }
  ...
  for(var i = 0;i<portCount;i++)
  {
    ...
    port.setBackgroundColor(new Color(115,245,115));
    port.setColor(new Color(115,115,115));
    port.setMaxFanOut(1);
    port.setName(values[i]);

    // Label des Ports bestimmen
    //
    port.decoLabel = new Label(values[i]+" =");
    port.decoLabel.setPosition(0,25+25*i-8);
    port.decoLabel.setDimension(newWidth-15,15);
    port.decoLabel.setWordwrap(false);
    port.decoLabel.setFontSize(8);
    port.decoLabel.setAlign("right");
    this.html.appendChild(port.decoLabel.getHTMLElement());
    if(this.workflow!=null)
    { 
      port.setWorkflow(this.workflow);
      this.addPort(port,this.getWidth(),25+25*i);
      port.paint();
    }
  }
}
