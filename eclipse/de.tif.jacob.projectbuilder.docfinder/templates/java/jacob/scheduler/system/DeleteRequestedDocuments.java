...
{
	static public final transient String RCS_ID = "$Id: DeleteRequestedDocuments.java,v 1.1 2007/11/25 22:11:24 freegroup Exp $";
	...
	public void run(TaskContextSystem context) throws Exception
	{
		...
		try
		{
			...
			int deletecount = documentTable.searchAndDelete(trans);
			trans.commit();

			if(deletecount>0)
				logger.info("Removing ["+deletecount+"] documents from database");
		}
		finally
		{
			trans.close();
