var candystick = {
	...
	add : function(object, options)
	{
		...
		var self, vo, width, height, tmp, id, defopts = {
			"width" : candystick.defaultWidth,
			"height" : candystick.defaultHeight,
			"pulse" : candystick.defaultPulse,
			"name" : candystick.defaultName,
			"limit" : candystick.defaultLimit,
			"vertical" : candystick.defaultVertical,
			"noscale" : candystick.defaultNoscale,
			"busy" : candystick.defaultBusy,
			"gradient" : candystick.defaultGradient,
			"scale" : candystick.defaultScale,
			"reflex" : candystick.defaultReflex,
			"opacity" : candystick.defaultOpacity,
			"empty" : candystick.defaultEmpty,
			"radius" : candystick.defaultRadius,
			"colors" : candystick.defaultColors,
			"values" : candystick.defaultValues
		};
		if (options)
		{
		...
		width = Math.max((typeof options['width'] === 'number' ? options['width'] : candystick.defaultWidth), (vo ? 8 : 48));
		height = Math.max((typeof options['height'] === 'number' ? options['height'] : candystick.defaultHeight), (vo ? 48 : 12));
		...
		id = (typeof options['name'] === 'string' ? options['name'] : candystick.defaultName);
		...
		if (!document.getElementById(tmp))
		{
			try
			{
				if (document.all && document.namespaces && !window.opera)
				{
					...
					{
						var e = [ "shape", "shapetype", "group", "background", "path", "formulas", "handles", "fill", "stroke", "shadow", "textbox", "textpath", "imagedata", "line", "polyline", "curve", "roundrect", "oval", "rect", "arc", "image" ], s = document
								.createStyleSheet();
				...
				if (self || self.getContext("2d"))
				{
					...
					candystick.modify(self, options);
					return self.id;
				} else
				{
					return false;
	...
	modify : function(self, options)
	{
		...
		function P(i)
		{
			var p = self.cl.concat(candystick.defaultColors), k = p.length - 1, t = i;
			if (t > k)
			{
				t = (i % k) - 1;
			}
		...
		if (self)
		{
			var i, q, g, c, n, a, z, l, t = 0, j = 0, m = 0, s = 0, e = 0, y = 0, x = 0, v = (self.width < self.height ? 1 : 0), r = parseInt(v ? self.width / 2 : self.height / 3), h = v ? self.height - r : r * 2, w = self.width, b = parseInt((v ? w : h) * 1.1), k = Math
					.round(((v ? h : w) - (2 * r)) / b);
			try
			{
				self.cc = (typeof options['empty'] === 'string' ? options['empty'] : self.options['empty']);
				self.cc = self.cc.match(/^#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]$/i) ? self.cc : candystick.defaultEmpty;
				...
				{
					var head, foot, fyll, flex, shadb, shadd, scalb = '', scald = '', j = 'm 0,0 l 0,' + parseInt((r - q) * 100) + ' qy ' + parseInt(q * 100) + ',' + parseInt(r * 100) + ' l ' + parseInt((w - q) * 100) + ',' + parseInt(r * 100) + ' qx ' + parseInt(w * 100) + ','
							+ parseInt((r - q) * 100) + ' l ' + parseInt(w * 100) + ',0 x e';
					head = '<v:group style="zoom:1;display:' + self.dpl + ';margin:0px;padding:0px;position:relative;width:' + self.width + 'px;height:' + self.height + 'px;" coordsize="' + self.width + ',' + self.height
							+ '"><v:rect filled="f" stroked="f" strokeweight="0" style="zoom:1;display:block;position:absolute;top:0px;left:0px;margin:0px;padding:0px;width:' + self.width + 'px;height:' + self.height + 'px;"></v:rect>';
					foot = '</v:group>';
					...
					flex = '<v:shape id="' + (self.id + '_flex') + '" coordorigin="0,0" coordsize="' + (w * 100) + ',' + (r * 100) + '" path="' + j + '" style="filter:Alpha(opacity=' + (100 - (100 * self.ty))
							+ ',finishOpacity=0,startX=0,finishX=0,startY=0,finishY=100,style=1); width:' + w + 'px;height:' + r + 'px;left:0px;top:' + h + 'px;position:absolute;display:block;margin:0px;padding:0px;rotation:180;" filled="f" stroked="f" strokeweight="0">';
					if (v)
					{
						...
						shadb = '<v:shape coordorigin="0,0" coordsize="'
								+ (r * 100)
								+ ','
								+ (h * 100)
								+ '" path="'
						...
						shadd = '<v:shape coordorigin="0,0" coordsize="'
								+ (r * 100)
								+ ','
								+ (h * 100)
								+ '" path="'
					...
					} else
					{
						...
						shadd = '<v:shape coordorigin="0,0" coordsize="' + (w * 100) + ',' + (r * 100) + '" path="' + j + '" style="width:' + w + 'px;height:' + r + 'px;left:0px;top:' + r
