...
public class GroupPropertyPlugin extends IPropertyPlugin
{
  ...
  public Object getPropertyValue(ObjectModel model, Object propName)
  {
    if(propName == DEFAULT_JAVA_PACKAGE_PROPERTY)
      return ((UIGroupModel)model).getChildrenDefaultJavaPackage();
  ...
  public boolean setPropertyValue(ObjectModel model, Object propName, Object val)
  {
    if(propName==DEFAULT_JAVA_PACKAGE_PROPERTY)
    {
      String oldPackage = ((UIGroupModel)model).getChildrenDefaultJavaPackage();
      if(oldPackage.equals(val))
        return true;
      
      try
      {
        ((UIGroupModel)model).setChildrenDefaultJavaPackage((String)val);
