CREATE OR REPLACE PROCEDURE sap_object_import
(
...
BEGIN
	...
	FOR edvinobj_rec IN (SELECT edvin_object.*, 
	                     ...
	                     WHERE sap_first.EDVIN_OBJECT=edvin_object.EXTERNAL_ID
	                           AND edvin_object.EXT_SYSTEM_KEY=edvin_ext_system_key 
	...
	UPDATE sap_first_objectimp SET error='Kein EDVIN-Objekt vorhanden' WHERE EDVIN_OBJECT NOT IN (SELECT EXTERNAL_ID FROM object WHERE EXT_SYSTEM_KEY=edvin_ext_system_key);
	
	-- do not commit since this will be done outside, i.e. by the caller
	-- COMMIT;
END sap_object_import;
