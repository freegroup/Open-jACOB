...
public class Thumbnail
{
   ...
   * because GIF is protected or something - so always make your outFilename end
   * in 'jpg'. PNG's with transparency are given white backgrounds
   */
  public static BufferedImage createThumbnail(Image inImage, int largestDimension) throws Exception
  {
      double scale;
      ...
      // original
      {
        int numSteps = Math.max(1, sizeDifference / 100);
        ...
        if (numSteps % 2 == 1) // if there's an odd number of steps
          centerStep = (int) Math.ceil((double) numSteps / 2d); // find the
        // center
        // step
        else
        ...
        for (Integer i = 0; i < numSteps; i++)
        {
          if (i + 1 != centerStep) // if this isn't the center step
          {
            ...
            {
              if (numSteps - i > numSteps / 2) // if we're in the first half
                // of the reductions
