...
{
  public final static String PROPERTYFILE_SUFFIX ="_docpool_restore_propertyfile_";
  ...
  private static void addFolder(Context context, ZipOutputStream out, IDataTableRecord folderRecord) throws Exception
  {
    ...
    // Complete the entry
    out.closeEntry();
    addPropertyfile(context, out, fileName+PROPERTYFILE_SUFFIX,  folderRecord.getSaveStringValue(Folder.pkey),  folderRecord.getSaveStringValue(Folder.synonyme));
  ...
  private static void addDocument(Context context, ZipOutputStream out, IDataTableRecord documentRecord) throws Exception
  {
    ...
    addPropertyfile(context, out, fileName+PROPERTYFILE_SUFFIX,  documentRecord.getSaveStringValue(Document.pkey),  documentRecord.getSaveStringValue(Document.synonyme));
  }
  
  private static void addPropertyfile(Context context, ZipOutputStream out, String fileName, String origPkey, String synonyme) throws Exception
  {
    Properties props = new Properties();
    props.setProperty(Recyclebin.original_pkey, origPkey);
    props.setProperty(Recyclebin.synonyme, synonyme);
    
    // Add ZIP entry to output stream.
    out.putNextEntry(new ZipEntry(fileName));
    props.store(out, "restoreable properties");
    out.closeEntry();
  }
}
