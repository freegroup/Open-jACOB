...
//  element.style.zIndex = ++zIndex;
  ...
  if (ondragfocus) this.ondragfocus(element);
 }
}};
...
DragResize.prototype.deselect = function(delHandles) { with (this)
{
 ...
 if (delHandles)
 {
  if (ondragblur) this.ondragblur(element);
...
DragResize.prototype.mouseDown = function(e) { with (this)
{
 ...
 if (newElement && (!element || (newElement == element)))
 {
  ...
  handle = newHandle;
  if (handle && ondragstart) this.ondragstart(element, hRE.test(handle.className));
 }
}};
...
DragResize.prototype.mouseMove = function(e) { with (this)
{
 ...
 mouseX = (mouseX/snapX).round()*snapX;
 mouseY = (mouseY/snapY).round()*snapY;
 ...
 elmX = (elmX/snapX).round()*snapX;
 elmY = (elmY/snapY).round()*snapY;
 elmW = (elmW/snapX).round()*snapX;
 elmH = (elmH/snapY).round()*snapY;
 ...
 if (ondragmove) this.ondragmove(element, isResize);
...
var Droppables = {
  ...
  fire: function(event, element) {
    ...
    Position.prepare();
    // FIX: Sonst klappt das DragDrop in einem Container mit Scrolling ncht richtig
...
var Draggable = Class.create({
  ...
  updateDrag: function(event, pointer) {
    ...
    if(!this.options.quiet){
      Position.prepare();
Position.includeScrollOffsets = true;      Droppables.show(pointer, this.element);
    }
  ...
  finishDrag: function(event, success) {
    ...
    if(this.options.quiet){
      Position.prepare();
Position.includeScrollOffsets = true;      var pointer = [Event.pointerX(event), Event.pointerY(event)];
  ...
  scroll: function() {
    ...
    Position.prepare();
Position.includeScrollOffsets = true;    Droppables.show(Draggables._lastPointer, this.element);
