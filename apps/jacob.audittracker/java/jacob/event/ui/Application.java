 ...
 * 
 * Created on Wed Feb 28 18:01:57 CET 2007
 */
...
{
	static public final transient String RCS_ID = "$Id: Application.java,v 1.3 2007/04/11 21:06:12 herz Exp $";
   ...
   */
	public void onCreate(IClientContext context, IApplication app)
	{
      ...
      table = context.getDataTable(jacob.model.Template.NAME);
      if(table.search()!=1)
      {
        IDataTableRecord record = table.newRecord(trans);
        InputStream is = ResourceProvider.class.getResourceAsStream("pruefblatt.rtf");
        byte[] document = IOUtils.toByteArray(is);
        is.close();
        record.setValue(trans, Template.name, "Vorlage Auditbericht");
        record.setValue(trans, Template.document, DataDocumentValue.create("Auditblatt.rtf", document));
      }
      trans.commit();
		}
		catch(Exception exc)
