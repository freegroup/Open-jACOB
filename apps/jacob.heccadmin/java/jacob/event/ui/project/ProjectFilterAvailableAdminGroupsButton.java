 ...
 * 
 * Created on Tue Jan 20 14:30:46 CET 2009
 */
...
/**
 * The event handler for the ProjectFilterAvailableAdminGroupsButton generic button.<br>
 * The {@link #onClick(IClientContext, IGuiElement)} will be called, if the user clicks on this button.<br>
 * 
 * @author R.Spoor
 */
public class ProjectFilterAvailableAdminGroupsButton extends IButtonEventHandler 
{
	static public final transient String RCS_ID = "$Id: ProjectFilterAvailableAdminGroupsButton.java,v 1.1 2009/02/17 15:23:32 R.Spoor Exp $";
    ...
    static void filterAvailableGroups(IClientContext context) throws Exception
    {
        ...
        String filter = group.getInputFieldValue("projectFilterAvailableAdminGroupsText");
        ...
        IDataTable table = accessor.getTable(Projectadmingroup.NAME);
        table.qbeClear();
        table.qbeSetValue(Projectadmingroup.name, filter);
        int count = table.search();
        Set<String> groups = new TreeSet<String>();
        for (int i = 0; i < count; i++)
        {
            IDataTableRecord record = table.getRecord(i);
            String groupname = record.getStringValue(Projectadmingroup.name);
