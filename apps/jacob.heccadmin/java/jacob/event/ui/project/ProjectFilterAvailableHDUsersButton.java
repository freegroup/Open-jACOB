 ...
 * 
 * Created on Thu Dec 18 15:25:32 CET 2008
 */
...
/**
 * The event handler for the ProjectFilterAvailableUsersButton generic button.<br>
 * The {@link #onClick(IClientContext, IGuiElement)} will be called, if the user clicks on this button.<br>
 * 
 * @author R.Spoor
 */
public class ProjectFilterAvailableHDUsersButton extends IButtonEventHandler 
{
	static public final transient String RCS_ID = "$Id: ProjectFilterAvailableHDUsersButton.java,v 1.1 2009/02/17 15:23:29 R.Spoor Exp $";
	...
	static void filterAvailableUsers(IClientContext context) throws Exception
	{
        IGroup group = context.getGroup();
        String filter = group.getInputFieldValue("projectFilterAvailableHDUsersText");
        ...
        IDataTable table = accessor.getTable(Projecthdadmin.NAME);
        table.qbeClear();
        table.qbeSetValue(Projecthdadmin.distinguishedname, filter);
        int count = table.search();
        Set<String> users = new TreeSet<String>();
        for (int i = 0; i < count; i++)
        {
            IDataTableRecord record = table.getRecord(i);
            String username = record.getStringValue(Projecthdadmin.name);
        ...
        available.setOptions(users.toArray(entries));
	}
