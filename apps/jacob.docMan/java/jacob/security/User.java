...
{
  static public final transient String RCS_ID = "$Id: User.java,v 1.1 2010-09-17 08:42:24 achim Exp $";
   ...
   *           on any problem
   */
  protected User(UserFactory factory, IDataTableRecord userRecord, boolean anonymous) throws GeneralSecurityException
  {
    ...
    try
    {
      ...
      else if (userRecord.getValue(Account.role).equals(Account.role_ENUM._administrator))
      {
        this.roles.add(Role.ADMIN);
        this.roles.add(Role.DEBUG);
        if (userRecord.getintValue(Account.editor)==1)
        {
          this.roles.add(Role.EDITOR);
        }
