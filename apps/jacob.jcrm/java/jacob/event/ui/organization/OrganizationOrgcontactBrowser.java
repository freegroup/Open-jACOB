package jacob.event.ui.organization;
 ...
 *
 */
public class OrganizationOrgcontactBrowser extends de.tif.jacob.screen.event.IBrowserEventHandler
{
  static public final transient String RCS_ID = "$Id: OrganizationOrgcontactBrowser.java,v 1.1 2005/10/12 15:29:01 sonntag Exp $";
   ...
   * 
   */
  public void onRecordSelect(IClientContext context, IBrowser browser, IDataTableRecord orgcontactRecord) throws Exception
  {
    // clear complete domain to fresh with given record
    context.clearDomain();

    context.setCurrentForm("contact");

    // constrain search with primary key of the given orgcontact
    //
    IDataTable contactTable = context.getDataTable("contact");
    contactTable.qbeSetPrimaryKeyValue(orgcontactRecord.getPrimaryKeyValue());

    // search and propagate the record
    //
    IDataBrowser contactBrowser = context.getDataBrowser("contactBrowser");
    contactBrowser.search("r_contact", Filldirection.BOTH);
    if (contactBrowser.recordCount() == 0)
    {
      // just in case the record has been deleted in the mean while
      throw new RecordNotFoundException(orgcontactRecord.getId());
    }
    contactBrowser.setSelectedRecordIndex(0);
    contactBrowser.propagateSelections();
  }
}
