...
{
  static public final transient String RCS_ID = "$Id: User.java,v 1.2 2007/02/07 07:50:59 freegroup Exp $";
  ...
  protected User(UserFactory factory, IDataTableRecord userRecord) throws GeneralSecurityException
  {
    ...
    try
    {
      ...
      this.password = userRecord.getStringValue(Account.password);
      this.mandatorId = userRecord.getStringValue(Account.mandator_id);
      // determine user roles
      //
      if (loginId.equals(UserFactory.GUEST_USER_ID))
      {
      ...
      // Falls es einen Raum gibt, in dem der User als Admin eingetragen ist, dann
      // bekommt er noch eine Rolle mehr
      //
      IDataAccessor acc = userRecord.getAccessor().newAccessor();
      IDataTable roomTable = acc.getTable(Room.NAME);
      roomTable.qbeSetKeyValue(Room.room_admin_key, this.key);
      if(roomTable.search()>0)
      	this.roles.add(Role.ROOMADMIN);
    }
    catch (Exception e)
    {
  ...
  {
  	if(hasRole(Role.GUEST.getName()))
  		throw new UserException("Unable to change password for guest user.");

  	if(loginId.equals(UserFactory.DEMO_USER_ID))
  		throw new UserException("Unable to change password for demo user.");
