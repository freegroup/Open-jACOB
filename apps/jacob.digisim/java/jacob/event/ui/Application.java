...
import jacob.model.Login_statistik;
...
{
	static public final transient String RCS_ID = "$Id: Application.java,v 1.2 2007/02/07 07:50:59 freegroup Exp $";
   ...
   */
	public void onCreate(IClientContext context, IApplication app)
	{
		...
		try
		{
			IDataTable statisticTable = acc.getTable(Login_statistik.NAME);
			IDataTableRecord statisticRecord = statisticTable.newRecord(trans);
			statisticRecord.setValue(trans, Login_statistik.loginid, context.getUser().getLoginId());
			trans.commit();
		}
		catch(Exception exc)
		{
