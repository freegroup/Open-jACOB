...
{
	static public final transient String RCS_ID = "$Id: EmailImport.java,v 1.4 2010/02/08 16:23:38 freegroup Exp $";
				    ...
				    for (int j = 0; j < froms.length; j++)
				    {
				      ...
				      if (from instanceof InternetAddress)
				      {
				      	...
				      	String email =  internetFrom.getAddress();
						    // falls die eMail Addresse nicht bekannt ist, dann wird diese �bersprungen
						    // (email whitelist)
				      	//
      ...
      if (contentType != null)
      {
        ...
        if (contentType.indexOf("text/plain") != -1)
        {
          if (content instanceof String)
          {
            ...
            if(body.length()>0)
            {
              // Falls die erste Zeile in dem Dokument eine URL ist, dann wird versucht
              // den Content dieser URL zu holen. Eventuell ist es ja ein Bild welches 
              ...
              if(lines.length==1 && lines[0].startsWith("http"))
              {
                try
                {
                  ...
                  String filename = FilenameUtils.getName(lines[0]);
                  InputStream stream = url.openStream();
                  byte[] urlContent = IOUtils.toByteArray(stream);
                  docfile = DataDocumentValue.create(filename, urlContent);
                  description = body;
                }
                catch(Exception exc)
                {
              ...
              {
                docfile = DataDocumentValue.create("text.txt", ((String)content).getBytes());
        ...
        if (docfile != null)
        {
            ...
            documentRecord.setStringValue(trans, Document.description, description);
						trans.commit();
					} 
        	catch (Exception e) 
					{
  ...
  {
  	// Die eMail ist nur gültig wenn diese in der white list vorhanden ist
  	//
