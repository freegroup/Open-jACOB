...
public abstract class GenericSendAs extends IButtonEventHandler
{
     ...
     * siehe auch Yan.NotificationAskCallback
     * 
     */
    static class AskCallback implements IFormDialogCallback
    {
        ...
        AskCallback(String document, String address, String xslStylesheet)
        {
        ...
        public void onSubmit(IClientContext context, String buttonId, Map formValues) throws Exception
        {
            ...
            try
            {
                ...
                record.appendToHistory(myTrans, "Nachricht wurde manuell versandt an: " + address);
                myTrans.commit();
            }
            finally
            {
    ...
    public class ComboBoxCallback implements IFormActionEmitter
    {
        ...
        /**
         * Falls in dem Dialog ein neuer Combobox Eintrag ausgewählt wird, wird
         * dieser Callback aufgerufen. Der Callback füllt in Abhängigkeit der
         * ComboBox 'msgTyp' und 'recipienType' den Defaultwert für die
         * Empfängeraddresse vor.<br>
         * 
         */
        public void onAction(IClientContext context, IFormDialog parent, Map formValues) throws Exception
        {
            ...
            String input = "<unset>";
            // Vorblenden der Empfängeraddresse in abhängigkeit von Type der
            // Meldung und
            // Typ des Empfängers
            //
            switch (type + group)
     ...
     * Dem Benutzter wird ein Dialog angezeit in dem folgende Auswahl getroffen
     * werden kann:<br>
     * <ul>
     ...
     * </lu> <br>
     * Nachdem der Benutzer 'Senden' drückt wird die ermittelte
     * Nachrichtenkonfiguration in die Tabelle 'yan_task' eingetragen. Dort wird
     * diese dann durch ein externes Programm 'YAN' abgearbeitet. <br>
     */
    public void onAction(IClientContext context, IGuiElement button) throws Exception
    {
        ...
        IFormDialog dialog = context.createFormDialog("Title", layout, new IFormDialogCallback()
        {
            public void onSubmit(IClientContext context, String buttonId, Map formValues) throws Exception
            {
                ...
                case 0:
                    ...
                    if (!validator.isValid(recipienAddress))
                    {
                        alert("Die Emailadresse ist ungültig - Meldung wurde nicht versendet.");
                        return;
                    }
                ...
                // nachfragen
                //
                Yan.fillAskFields(context, auftrag, new AskCallback(auftrag, recipienAddress, stylesheet));
        ...
        dialog.addLabel("Typ:", cc.xy(2, 3));
        dialog.addComboBox(new ComboBoxCallback(), "msgType", new String[]
        { "eMail", "Fax", "SMS" }, 0, cc.xy(4, 3));

        dialog.addHeader("Empfänger", cc.xywh(1, 5, 4, 1));
        dialog.addLabel("Typ:", cc.xy(2, 7));
        dialog.addComboBox(new ComboBoxCallback(), "recipienType", new String[]
        { "Gruppe", "Kunde", "Centerleiter" }, 0, cc.xy(4, 7));

        // eMail an Kunde wird vorgeblendet
        // String
        // defaultAddress=context.getDataTable("customerint").getSelectedRecord().getStringValue("emailcorr");
        ...
        dialog.addLabel("Adresse:", cc.xy(2, 9));
        dialog.addTextField("recipienAddress", defaultAddress, cc.xy(4, 9));
