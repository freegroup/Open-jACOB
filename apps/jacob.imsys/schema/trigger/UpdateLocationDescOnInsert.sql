...
if note_existing > 0 Then
	if fapliskey_existing > 0 then
		   :new.description := :new.note || '-' || name_site || '/' || name_sitepart || '/' || name_building || '/' || name_buildingpart || '/' || name_floor || '/' 
|| name_plane || '/' || name_room || '/' || name_baxis || '-' || name_zaxis;
	else
		 :new.description := :new.note;
	end if;
else    
	if fapliskey_existing > 0 then
	   :new.description := name_site || '/' || name_sitepart || '/' || name_building || '/' || name_buildingpart || '/' || name_floor || '/' || name_plane || '/' || 
name_room || '/' || name_baxis || '-' || name_zaxis;
	else
 	  :new.description := '///////-';
	end if;   
end if;   
