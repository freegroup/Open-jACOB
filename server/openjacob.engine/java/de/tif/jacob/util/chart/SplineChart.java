...
public class SplineChart extends GenericChart
{
  public SplineChart(int width, int height)
  {
    super(width,height);
  }
  ...
  {
    // plot data
    Color labelColor = backgroundColor1.darker().darker();
    
    g.setFont(font);
    double x1 = paddingLeft;
    double y1=0;
    double xInc = (getWidth() - paddingLeft - paddingRight) / (getData().length-1);
  ...
  {
    // plot data
    double x1 = paddingLeft + getPaddingLeftOffset();
    double x2=0;
    double y1=0;
    double y2=0;
    double xInc = (getWidth() - paddingLeft - paddingRight) / (getData().length-1);
    ...
    for(int j = 0; j < getData().length; j++)
    {
        y1 = paddingTop + (getHeight() - baddingBottom-paddingTop) - (getData()[j] - dataVals[0]) * yScale;
        if(j > 0)
  ...
  public Rectangle[] getFulcrumBounds()
  {
    ...
    // plot data
    double xInc = (getWidth() - paddingLeft-paddingRight-getFulcrumWidth()) / (getData().length-1);
