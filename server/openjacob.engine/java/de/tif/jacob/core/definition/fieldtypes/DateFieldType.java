 ...
 */
public final class DateFieldType extends FieldType
{
  static public final transient String RCS_ID = "$Id: DateFieldType.java,v 1.11 2010/08/11 22:32:54 ibissw Exp $";
  ...
  private final String defaultValue;
	 ...
	 * 
	 */
  public DateFieldType(String defaultValue)
  {
    this.defaultValue = defaultValue;
  }

  public DateFieldType(DateField type)
  {
  ...
  public QBEExpression createQBEExpression(DataSource dataSource, QBEFieldConstraint constraint) throws InvalidExpressionException, IllegalArgumentException
  {
    ...
    {
      return QBEExpression.parseDate((String) value);
    }
    else if (value instanceof Date)
    {
      return new QBELiteralExpression(new QBEDateLiteral((Date) value));
    }
    else
    {
  ...
  /**
   * Adjust the time value by means of reseting time part.
   * 
   * @param time millis since 1.1.1970
   * @return the adjusted date
   */
  private Date adjust(long time)
  {
    ...
    if (object instanceof String)
    {
      ...
      if (expression.endsWith("L"))
      {
        try
        {
          return new Date(Long.parseLong(expression.substring(0, expression.length() - 1)));
        }
    	...
    	return DatetimeUtil.convertToDate(expression);
    }
  ...
  public int sortDataValueNotNull(Object dataValue1, Object dataValue2, Collator collator)
  {
	...
	{
		return DatetimeUtil.convertToDate(this.defaultValue);
	}
	...
	{
    statement.setDate(index, (Date) dataValue); 
  }
    ...
    tableField.getCastorTableFieldChoice().setDate(type);
  }

	public String convertDataValueToString(Object value, Locale locale, int style)
  {
	  if (style == IDataRecord.RAW_STYLE && value != null)
      return Long.toString(((Date) value).getTime()) + "L";
	  
    return DatetimeUtil.convertDateToString((Date) value, locale);
  }
	...
	{
		return new DateInputFieldDefinition(name,null, null, null, position, visible, readonly, tabIndex, paneIndex, caption, localTableAlias, localTableField, null);
	}
