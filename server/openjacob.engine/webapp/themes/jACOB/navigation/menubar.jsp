...
%>
<script>
var expandCurrentImage = "themes/jACOB/images/ns-expand_current.png";
var expandImage        = "themes/jACOB/images/ns-expand.png";
var collapseImage      = "themes/jACOB/images/ns-collapse.png";
...
<div class="listbar_header"><div id="header_login"><%=context.getUser().getFullName()%> <br>
<%
    ...
    if(context.getUser().isAnonymous() && file.exists()){  %>
        [ <a href=".<%=appLogin%>"><j:i18n id="BUTTON_LOGIN"/></a> ]</div>
    <%}else{%>
        [ <a href="#" onClick="FireEvent(<%=exitButton.getId()%>,'click');"><j:i18n id="BUTTON_TOOLBAR_CLOSE"/></a> ]</div>
    <%}%>
    <% if(app.hasSpotlightSupport()){ %> 
    ...
    <%
          ...
          while (iter.hasNext())
          {
            ...
            if (domain.getCanCollapse() == false)
            {
              if (!hideDomainLabel)
              {
                out.println("<div><table width='100%'><tr><td width='100%' class=\"simpleNavigationLevel1Normal\"  style='cursor:default' >");
            ...
            else
            {
              ...
              if (!hideDomainLabel)
              {
                ...
                //
                out.println("<div><table width='100%'><tr onclick=\"toogleNavigation(" + domain.getId() + ",'" + domain.getEtrHashCode() + "'," + isCurrentDomain + ");\"><td width='100%' class=\"simpleNavigationLevel1Normal\" style='cursor:pointer;'>");
                out.println(StringUtil.htmlEncode(label));
                out.println("</td><td  align='right'>");
                if (Boolean.TRUE.equals(domain.isOpen()))
                {
                  out.println("<img style='cursor:pointer;' id='image_" + domain.getId() + "' src='themes/jACOB/images/ns-collapse.png'>");
                  out.println("</td></tr></table></div>");
                  out.println("<div id=\"" + domain.getEtrHashCode() + "\" >");
                }
                else
                {
                  if (domain == context.getDomain())
                    out.println("<img style='cursor:pointer;' id='image_" + domain.getId() + "' current='true' src='themes/jACOB/images/ns-expand_current.png'>");
                  else
                    out.println("<img style='cursor:pointer;' id='image_" + domain.getId() + "' src='themes/jACOB/images/ns-expand.png'>");
                  out.println("</td></tr></table></div>");
              ...
              for (int i = 0; i < entries.length; i++)
              {
                ...
                if (onClickAction != null)
                {
                  ...
                  {
                    if (domain.isCurrentNavigationEntry(entry) && domain == context.getDomain())
                      out.print(" class='simpleNavigationLevel1Selected' ");
                    else
                      out.print(" class='simpleNavigationLevel1Normal' ");
                  }
                  else
                  {
                    if (domain.isCurrentNavigationEntry(entry) && domain == context.getDomain())
