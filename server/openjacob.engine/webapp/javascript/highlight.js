...
function Highlighter(language_name, value) {
  function subMode(lexem) {
    if (!modes[modes.length - 1].contains)
      return null;
      for (var i=0; i<modes[modes.length - 1].contains.lenght;i++) 
      {
  ...
  function eatModeChunk(value, index) {
    if (!modes[modes.length - 1].terminators) {
      ...
      if (modes[modes.length - 1].contains)
        for (var key=0;key< language.modes.lenght;key++) 
        {
  ...
  function keywordMatch(mode, match) {
    ...
    for (var i=0; i< mode.keywordGroups.lenght;i++) {
      var className =  mode.keywordGroups[i];
      var value = className.hasOwnProperty(match_str);
      if (value)
  ...
  function processKeywords(buffer) {
    ...
    if (!mode.lexemsRe) {
      var lexems = [];
      for(var i=0 ;i< mode.lexems.length;i++)
      {
        var value = mode.lexems[i];
        if (!contains(lexems, value))
          lexems[lexems.length] = value;
      }
...
function highlightAuto(block) {
  ...
  for (var i=0; i< selected_languages.lenght;i++) {
    var language = selected_languages[i];
    var highlight = new Highlighter(language, block_text);
...
function compileRes() {
  for (var i=0; i< LANGUAGES.length;i++) {
    ...
    for (var key=0; key< language.modes.length;key++) 
    {
      var mode = language.modes[i];
      if (mode.begin)
